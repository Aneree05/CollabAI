...
import { activityApi, fileApi, messageApi, projectApi, reviewApi } from '../../services/collabApi'
...
const WorkspacePage = () => {
  ...
  const handleSendMessage = () => {
    ...
    sendMessageMutation.mutate({ projectId, message: message.trim() })
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {['overview', 'chat', 'files', 'activity', 'reviews'].map((tab) => (
      ...
      {activeTab === 'chat' && (
        <Card className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button type="button" onClick={handleSendMessage} disabled={sendMessageMutation.isPending}>
              {sendMessageMutation.isPending ? <LoadingSpinner label="Sending..." /> : 'Send'}
            </Button>
          </div>
        </Card>
      )}
        ...
        <Card>
          <form onSubmit={handleSubmitReview} className="space-y-4">
            <div>
              ...
              <select className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none focus:border-primary" value={reviewRating} onChange={(event) => setReviewRating(event.target.value)}>
                {[1, 2, 3, 4, 5].map((value) => (
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-text">Comment</label>
              <textarea className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text outline-none focus:border-primary" rows="4" value={reviewComment} onChange={(event) => setReviewComment(event.target.value)} placeholder="Share your feedback about the project." />
            </div>
            <Button type="submit" disabled={reviewMutation.isPending}>
              {reviewMutation.isPending ? <LoadingSpinner label="Submitting..." /> : 'Submit Review'}
            </Button>
          </form>
        </Card>
      )}
